...
def estampar_firma_en_pdf(pdf_path, signature_b64, signer_index, email_user, nombre_user, ip_user):
    ...
    for page in doc:
        ...
        if instancias:
            ...
            # 2. ESCRIBIR NOMBRE REAL (Usamos "helv" normal)
    ...
    if signer_index == 1:
        ...
        audit_page.insert_text((50, 40), "CONSTANCIA DE CONSERVACIÓN INTERNA RALOY", fontsize=14, fontname="hebo", color=(0, 0, 0))
    else:
    ...
    if signer_index > 1:
        audit_page.draw_line(fitz.Point(50, y_inicio - 15), fitz.Point(550, y_inicio - 15), color=(0.8, 0.8, 0.8), width=1)

    # Insertamos los datos de la firma apilados
    audit_page.insert_text((50, y_inicio), f"Firma {signer_index} - Autenticado:", fontsize=11, fontname="hebo", color=(0, 0, 0))
