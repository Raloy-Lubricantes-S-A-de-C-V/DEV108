...
print("Doc vars keys:", list(json.loads(proc.get("document_variables")).keys())[:10])

# Check intersection
doc_vars = json.loads(proc.get("document_variables"))
for key in doc_vars.keys():
    if key in content_option:
        print("MATCH:", key, content_option[key])
